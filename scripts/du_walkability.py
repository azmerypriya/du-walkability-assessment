...
PATH_BUFFER_DEG    = 0.00004   # ~4 m path buffer for isochrone area polygon
...
def snap_all_pois(pois, segments, G):
    ...
    for poi in pois:
        ...
        for seg in segments:
            ...
            for i in range(len(seg) - 1):
                ...
                if d < min_dist:
                    ...
                    best_u        = u_coord
                    best_v        = v_coord
                    best_d_from_u = haversine(u_coord[0], u_coord[1],
                                              snap_pt.x,  snap_pt.y)
        ...
        G.add_node(poi_node, lon=poi['lon'], lat=poi['lat'],
                   name=poi['name'], poi_type=poi['type'])
...
def metric_distance_matrix(G, pois, poi_nodes):
    ...
    for i in range(n):
        for j in range(n):
            ...
            ed = haversine(pois[i]['lon'], pois[i]['lat'],
                           pois[j]['lon'], pois[j]['lat'])
            euc_matrix[i, j] = ed
    ...
    return {
        ...
        'mean_net_dist_m':   round(float(np.mean(valid_net)), 1) if len(valid_net) else np.nan,
        'median_net_dist_m': round(float(np.median(valid_net)), 1) if len(valid_net) else np.nan,
        'mean_detour':       round(float(np.mean(valid_det)), 4) if len(valid_det) else np.nan,
        ...
        for c2 in cats[ci:]:
            ...
            if values:
                ...
                results[key] = {'mean': np.nan, 'median': np.nan,
                                'std': np.nan, 'n_pairs': 0}
    return results


# ── M6 : Walkable Catchment Area / Isochrone Coverage ──────

def metric_isochrone_coverage(G, poi_nodes, campus_geom,
                               thresholds_m=(ISO_5MIN_M, ISO_10MIN_M)):
    """
    ...
    for poi_node in poi_nodes:
        ...
        for t in thresholds_m:
            ...
            for u, v, data in G.edges(data=True):
                ...
                def is_coord_node(n):
                    return (isinstance(n, tuple) and len(n) == 2
                            and isinstance(n[0], float) and isinstance(n[1], float))
    ...
    for t in thresholds_m:
        if iso_buffers[t]:
            ...
            results[t] = {'coverage_pct': 0.0,
                          'union_polygon': None, 'clipped_polygon': None}
    return results
...
def plot_overview(pois, segments, campus_geom, snap_coords,
                  snap_dists, poi_nodes, out_path):
    ...
    for poi, sc, sd in zip(pois, snap_coords, snap_dists):
        ...
        ax.plot([poi['lon'], sc[0]], [poi['lat'], sc[1]],
                color=color, lw=0.8, ls='--', alpha=0.6)

    # POIs
    for poi in pois:
        ax.scatter(poi['lon'], poi['lat'],
                   c=CAT_COLOR[poi['type']],
                   marker=CAT_MARKER[poi['type']],
                   s=80, zorder=5, edgecolors='white', linewidths=0.8)
        ax.annotate(poi['name'], (poi['lon'], poi['lat']),
                    textcoords='offset points', xytext=(4, 3),
                    fontsize=5.5, color='#212121', zorder=6)

    # Legend
    handles = [mpatches.Patch(color=CAT_COLOR[c], label=c.title())
    ...
    handles += [
        ...
        plt.Line2D([0], [0], color='#43a047', lw=0.8, ls='--', label='Snap ≤50 m (served)'),
        plt.Line2D([0], [0], color='#e53935', lw=0.8, ls='--', label='Snap >50 m (unserved)'),
    ]
    ax.legend(handles=handles, loc='lower right', fontsize=7, framealpha=0.9)

    ax.set_title('University of Dhaka — Footpath Network and Points of Interest',
...
def plot_distance_heatmap(matrix, pois, title, out_path, cmap='YlOrRd',
                           fmt='.0f', unit=''):
...
def plot_isochrone_map(pois, segments, campus_geom, iso_results, out_path):
    ...
    titles    = {ISO_5MIN_M:  '5-Minute Isochrone (420 m)',
    ...
    for ax, t in zip(axes, [ISO_5MIN_M, ISO_10MIN_M]):
        ...
        ax.set_title(f"{titles[t]}\nCampus Coverage: {cov:.1f}%",
                     fontsize=11, fontweight='bold')
        ax.set_xlabel('Longitude', fontsize=8)
        ax.set_ylabel('Latitude', fontsize=8)
        ax.tick_params(labelsize=7)

    fig.suptitle('Walkable Catchment Area Analysis — University of Dhaka',
                 fontsize=14, fontweight='bold', y=1.01)

    handles = [mpatches.Patch(color=CAT_COLOR[c], label=c.title())
               for c in ['academic', 'residential', 'service']]
    handles += [mpatches.Patch(color=colors[ISO_5MIN_M], alpha=0.4, label='5-min reach'),
                mpatches.Patch(color=colors[ISO_10MIN_M], alpha=0.4, label='10-min reach')]
...
def plot_zone_directness(zone_results, out_path):
    ...
    # Filter out NaN
    valid = [(l, m, s, n) for l, m, s, n in zip(labels, means, stds, ns)
             if not np.isnan(m)]
    if not valid:
        return
    ...
    ax.set_title('Path Directness by Functional Zone\n(Detour Index ≥ 1.0; lower is better)',
...
def plot_par_bars(pois, snap_dists, out_path):
    ...
    bars     = ax.barh(range(len(names)), dists, color=colors, edgecolor='white', height=0.7)
    ax.set_yticks(range(len(names)))
    ax.set_yticklabels(names, fontsize=7.5)
    ax.axvline(PAR_THRESHOLD_M, color='#e65100', lw=1.5, ls='--',
               label=f'PAR threshold ({PAR_THRESHOLD_M} m)')
    ax.set_xlabel('Distance to Nearest Footpath (metres)', fontsize=10)
    ax.set_title('POI Snap Distances — Pedestrian Accessibility Assessment',
                 fontsize=12, fontweight='bold')
    ax.legend(fontsize=9)
...
def print_report(m1, m2, m3, m4, m5, m6, pois):
    ...
    print(f"{'─'*65}")
    print(f"  Connected pairs           : {m2['connected_pairs']} / {m2['total_pairs']}")
    ...
    print(f"{'─'*65}")
    print(f"  Total footpath length     : {m4['total_length_m']:.1f} m  "
    ...
    tier = 'Low (<5,000)' if nd < 5000 else 'Medium (5,000–10,000)' if nd < 10000 else 'High (>10,000)'
    print(f"  Benchmark tier            : {tier}")

    print(f"\n{'─'*65}")
    print("M5 — Functional-Zone Path Directness")
    print(f"{'─'*65}")
    print(f"  {'Zone Pair':<30s} {'Mean DI':>8s}  {'Median':>8s}  {'Std':>6s}  {'N':>4s}")
    print(f"  {'─'*30}  {'─'*8}  {'─'*8}  {'─'*6}  {'─'*4}")
    for key, vals in m5.items():
        if vals['n_pairs'] > 0:
            print(f"  {key:<30s} {vals['mean']:8.4f}  {vals['median']:8.4f}  "
    ...
    for t, res in m6.items():
        min_ = t / 60
        print(f"  {min_:.0f}-min isochrone ({t:.0f} m)   : {res['coverage_pct']:.2f}% of campus area")
...
def main():
    ...
    print(f"      {len(segments)} footpath segments | {len(pois)} POIs | campus ≈ {campus_area_m2/1e6:.4f} km²")
    ...
    print(f"      Max snap: {max(snap_dists):.1f} m | Min snap: {min(snap_dists):.1f} m | "
    ...
    print(f"      5-min coverage: {m6[ISO_5MIN_M]['coverage_pct']:.1f}% | "
          f"10-min coverage: {m6[ISO_10MIN_M]['coverage_pct']:.1f}%")

    print("[6/7] Generating figures …")
    plot_overview(pois, segments, campus_geom, snap_coords, snap_dists,
                  poi_nodes, f"{OUTPUT_DIR}/fig1_overview.png")
    plot_par_bars(pois, snap_dists, f"{OUTPUT_DIR}/fig2_par.png")
    plot_distance_heatmap(m3['net_matrix'], pois,
                          'Network Shortest-Path Distance Matrix (metres)',
                          f"{OUTPUT_DIR}/fig3_net_dist.png",
                          cmap='YlOrRd', fmt='.0f', unit='metres')
    plot_distance_heatmap(m3['detour_matrix'], pois,
                          'Detour Index Matrix (network / straight-line distance)',
                          f"{OUTPUT_DIR}/fig4_detour.png",
                          cmap='RdYlGn_r', fmt='.2f', unit='Detour Index')
    plot_zone_directness(m5, f"{OUTPUT_DIR}/fig5_zone_directness.png")
    plot_isochrone_map(pois, segments, campus_geom, m6,
                       f"{OUTPUT_DIR}/fig6_isochrones.png")

    print("[7/7] Results summary")
    print_report(m1, m2, m3, m4, m5, m6, pois)

    return m1, m2, m3, m4, m5, m6, pois, poi_nodes, G, segments, campus_geom, snap_dists, snap_coords
